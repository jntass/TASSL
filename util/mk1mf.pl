...
and [options] can be one of
	...
	no-srp					- No SRP
	no-ec					- No EC
	no-cnsm					- No CNSM(China SM Scheme)
	no-ecdsa				- No ECDSA
	no-ecdh					- No ECDH
	no-engine				- No engine
	no-hw					- No hw
...
$cflags.=" -DOPENSSL_NO_CNSM"   if $no_cnsm;
...
sub var_add
	...
	return("") if $no_cnsm   && $dir =~ /\/sm(2|3|4)/;
	...
	@a=grep(!/^e_sm1$/,@a) if $no_cnsm;
	@a=grep(!/^e_sm4$/,@a) if $no_cnsm;
	@a=grep(!/^e_ssf33$/,@a) if $no_cnsm;
